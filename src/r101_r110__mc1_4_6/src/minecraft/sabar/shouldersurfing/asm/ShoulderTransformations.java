...
import sabar.shouldersurfing.ShoulderSurfing;
...
public class ShoulderTransformations implements IClassTransformer
{
    ...
    public ShoulderTransformations()
    {
        ...
        registerMapping("EntityRendererClass",     "net.minecraft.client.renderer.EntityRenderer", "ban");
        registerMapping("EntityRendererJavaClass", "net/minecraft/client/renderer/EntityRenderer", "ban");
        registerMapping("EntityLivingJavaClass",   "net/minecraft/entity/EntityLiving"           , "md");
        ...
        registerMapping("ShoulderHelperJavaClass", "sabar/shouldersurfing/ShoulderHelper"        , "sabar/shouldersurfing/ShoulderHelper");
        ...
        registerMapping("clippingHelperImplJavaClass",     "net/minecraft/client/renderer/culling/ClippingHelperImpl", "bbe");
        registerMapping("clippingHelperJavaClass",         "net/minecraft/client/renderer/culling/ClippingHelper"    , "bbg");
         ...
         */
        registerMapping("GuiIngameClass",              "net.minecraft.client.gui.GuiIngame", "atr");
        registerMapping("GuiIngameJavaClass",          "net/minecraft/client/gui/GuiIngame", "atr");
        registerMapping("GuiJavaClass",                "net/minecraft/client/gui/Gui"      , "ats");
         ...
         */
        registerMapping("RenderPlayerClass",           "net.minecraft.client.renderer.entity.RenderPlayer", "bco");
        registerMapping("RenderPlayerJavaClass",       "net/minecraft/client/renderer/entity/RenderPlayer", "bco");
        registerMapping("renderPlayerMethod",          "renderPlayer"                                     , "a");
        registerMapping("EntityPlayerJavaClass",       "net/minecraft/entity/player/EntityPlayer"         , "qx");
    }
    ...
    private byte[] transformRenderPlayerClass(byte[] bytes, HashMap hm)
    {
        ...
        while (methods.hasNext())
        {
            ...
            if (m.name.equals(hm.get("renderPlayerMethod")) && m.desc.equals("(L" + hm.get("EntityPlayerJavaClass") + ";DDDFF)V"))
            {
				...
				hackCode.add(new FieldInsnNode(GETSTATIC, "sabar/shouldersurfing/ShoulderHelper", "SKIP_PLAYER_RENDER", "Z"));
        ...
        while (methods.hasNext())
        {
            ...
            if (m.name.equals(hm.get("renderGameOverlayMethod")) && m.desc.equals("(FZII)V"))
            {
				...
				if(offset == -1){
					...
					hackCode.add(new MethodInsnNode(INVOKESTATIC, "sabar/shouldersurfing/ShoulderHelper", "drawCrosshairs", "(L" + (String) hm.get("GuiJavaClass") + ";)V"));
        ...
        while (methods.hasNext())
        {
            ...
            if (m.name.equals(hm.get("orientCameraMethod")) && m.desc.equals("(F)V"))
            {
				...
				if(offset == -1){
					...
					hackCode.add(new MethodInsnNode(INVOKESTATIC, "sabar/shouldersurfing/ShoulderHelper", "getShoulderRotation", "()F"));
					hackCode.add(new InsnNode(FADD));
					hackCode.add(new VarInsnNode(FSTORE, 13));
					hackCode.add(new VarInsnNode(DLOAD, 10));
					hackCode.add(new MethodInsnNode(INVOKESTATIC, "sabar/shouldersurfing/ShoulderHelper", "getShoulderZoomMod", "()F"));
				...
				if(offset == -1){
					...
					hackCode.add(new MethodInsnNode(INVOKESTATIC, "sabar/shouldersurfing/ShoulderHelper", "verifyReverseBlockDist", "(D)V"));
					hackCode.add(new LabelNode(new Label()));
				...
				if(offset == -1){
					...
					ShoulderSurfing.logger.info("Located offset @ " + offset);
					InsnList hackCode = new InsnList();
					hackCode.add(new MethodInsnNode(INVOKESTATIC, "sabar/shouldersurfing/ShoulderHelper", "calculateRayTraceProjection", "()V"));
					hackCode.add(new LabelNode(new Label()));
					m.instructions.insertBefore(m.instructions.get(offset+1), hackCode);
